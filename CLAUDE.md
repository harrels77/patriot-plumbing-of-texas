...
The card people see when the site is shared (WhatsApp, iMessage, LinkedIn, …) is generated by `src/app/opengraph-image.tsx` via `next/og`, prerendered at build time and inherited by every page.

- The brand mark leads the card. It sits in an asymmetric split — logo left, type right — over cream, closed by a navy bar carrying the phone number and the Mon–Fri hours. Never let the card imply 24/7.
- The claim is stated as **family-owned** (eyebrow) plus **forty years** (tagline). Both halves belong on the card.
- The logo is read off disk and inlined as a base64 data URI: Satori cannot fetch assets over the network. Fraunces and IBM Plex Mono are fetched from Google Fonts at build time with an old User-Agent, because Satori parses `.woff` but not `.woff2`.
- Do NOT add `openGraph.images` to `layout.tsx`. The file convention wins for `og:image`, Next mirrors it to `twitter:image` on its own, and a metadata entry only creates a second, conflicting image.
- The card is baked at build time, so a change only reaches shared links after a deploy — and WhatsApp caches previews per URL, so verify with a fresh `?v=N` query.
